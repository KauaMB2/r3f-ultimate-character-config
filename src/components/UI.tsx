import { PHOTO_POSES, UI_MODES, useConfiguratorStore } from "../store.js";
...
import { LockedGroupInfo } from "../store.js";
import { PhotoPose } from "../store.js";
...
const PosesBox = () => {
  ...
  return (
    <div className="pointer-events-auto md:rounded-t-lg bg-gradient-to-br from-black/30 to-indigo-900/20  backdrop-blur-sm drop-shadow-md flex p-6 gap-3 overflow-x-auto noscrollbar">
      {(Object.keys(PHOTO_POSES) as PhotoPose[]).map((pose) => (
        <button
          key={pose}
  ...
  return (
    <div className="md:rounded-t-lg bg-gradient-to-br from-black/30 to-indigo-900/20  backdrop-blur-sm drop-shadow-md flex flex-col py-6 gap-3 overflow-hidden ">
      <div className="flex items-center gap-8 pointer-events-auto noscrollbar overflow-x-auto px-6 pb-2">
        {categories.map((category: CustomizationGroup) => (
          <button
            key={category.id}
            onClick={() => setCurrentCategory({ ...category, assets: category.assets || [] })}
            className={`transition-colors duration-200 font-medium flex-shrink-0 border-b ${
      ...
      {currentCategory?.name && lockedGroups[currentCategory.name] && (
        <p className="text-red-400 px-6">
          Asset is hidden by{" "}
          {lockedGroups[currentCategory.name]!
            .map((asset: LockedGroupInfo) => `${asset.name} (${asset.categoryName})`)
            .join(", ")}
        </p>
      )}
      <div className="flex gap-2 overflow-x-auto noscrollbar px-6">
        ...
        {currentCategory?.assets?.map((asset: CustomizationAsset) => (
          <button
            key={asset.id}
...
const ColorPicker = () => {
  ...
  if (!currentCategory?.name || !customization[currentCategory.name]?.asset) {
    return null;
  }
  ...
  return (
    <div className="pointer-events-auto relative flex gap-2 max-w-full overflow-x-auto backdrop-blur-sm py-2 drop-shadow-md noscrollbar px-2 md:px-0">
      {currentCategory.colorPalette?.colors?.map((color: string, index: number) => (
        <button
          key={`${index}-${color}`}
...
export const UI = () => {
  ...
  return (
    <main className="pointer-events-none fixed z-10 inset-0 select-none">
      <div
        className={`absolute inset-0 flex-col gap-3 bg-black z-10 pointer-events-none flex items-center justify-center transition-opacity ${
      ...
      >
        <img
          ...
          alt="Loading logo"
        />
        <p className="text-lg text-cyan-50 animate-pulse">Carregando...</p>
      </div>
